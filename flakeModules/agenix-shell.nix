    ...
    options.agenix-shell = {
      ...
      secretsPath = mkOption {
        ...
        default = if stdenv.hostPlatform.isLinux then
          ''/run/user/$UID/agenix-shell/$(${cfg.flakeNameCommand})/$(uuidgen)''
        else if stdenv.hostPlatform.isDarwin then
          ''$TMPDIR/agenix-shell/$(${cfg.flakeNameCommand})/$(uuidgen)''
        else
          throw "Unsupported system: ${pkgs.stdenv.system}";
        defaultText = lib.literalExpression ''"/run/user/$(UID)/agenix-shell/$(''${config.agenix-shell.flakeNameCommand})/$(uuidgen)"'';
